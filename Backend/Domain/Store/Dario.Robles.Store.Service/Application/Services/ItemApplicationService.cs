...
namespace Dario.Robles.Store.Service.Application.Services
{
    ...
    {
        public async Task<GetItemsForOrderResult> GetItemsForOrderAsync(Guid itemId)
        {
            GetItemsForOrderResult result = new GetItemsForOrderResult();
            var itemsForOrderFromRepo = await _unitOfWork.Items.GetItemsForOrderAsync(itemId);
            ...
            result.LinkedCollectionResource = _itemLinksBuilder.CreateDocumentationLinksForItems(itemId, wrapper);
